...
<main class="w-4/5 mx-auto mt-16 mb-24 bg-[#2A2A2A] bg-opacity-90 backdrop-blur-md rounded-lg shadow-2xl">
    <div class="p-8">
        <h2 class="text-3xl font-extrabold text-center text-[#81D4FA] mb-8">お問い合わせ</h2>
        <form action="{{ route('contact') }}" method="POST" class="max-w-lg mx-auto">
            @csrf
            <div class="mb-6">
                <label for="name" class="block text-sm font-semibold mb-2 text-[#E0E0E0]">お名前</label>
            ...
            <div class="mb-6">
                <label for="message" class="block text-sm font-semibold mb-2 text-[#E0E0E0]">お問い合わせ内容</label>
            ...
            <div class="flex justify-center">
                <button type="submit" class="bg-[#81D4FA] hover:bg-[#4FC3F7] text-[#121212] font-bold py-3 px-8 rounded-lg shadow-md transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#81D4FA]">
                    送信
                </button>
            </div>
        </form>
